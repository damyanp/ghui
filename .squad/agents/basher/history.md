...
- 📌 Two-step preservation test pattern: to verify `set_X` doesn't clobber fields set by `set_Y`, call both on an `AppState` with fields/work_items populated, then assert `state.view_config_cache()` holds both values (private access OK from `mod tests` child module).
...
- 📌 Task 6 (2026-05-19): The call-site swap from NodeBuilder to RecipeNodeBuilder was a single-line change in AppState::refresh(). The real work was auditing nodebuilder_tests for coverage gaps — two tests (filter-closed behavior and new-item-after-update via WorkItems::update()) were NOT covered by Task 2 snapshots and were ported to recipe_builder.rs before NodeBuilder was deleted. Default-recipe equivalence is CONDITIONAL on well-formed projects (where parent/child share the same Epic); Task 2 snapshot tests already cover `Pivot(Epic) → Hierarchy` so the equivalence was considered verified.
